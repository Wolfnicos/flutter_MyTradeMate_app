...
class LivePill extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Row(
    ...
    children: [
      ...
      Text(live ? 'Live' : 'Offline', style: const TextStyle(fontWeight: FontWeight.w600)),
    ],
  );
}
...
class SegTabs extends StatelessWidget {
  final int index; final ValueChanged<int> onChanged; final List<String> tabs;
  const SegTabs({super.key, required this.index, required this.onChanged, required this.tabs});
  @override Widget build(BuildContext context) => Container(
    decoration: BoxDecoration(
      ...
      for (int i=0;i<tabs.length;i++) Expanded(
        child: InkWell(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: i==index ? AppTheme.accent(0.25) : Colors.transparent,
              borderRadius: BorderRadius.circular(18),
            ),
            child: Center(child: Text(tabs[i], style: TextStyle(
              fontWeight: FontWeight.w700, color: i==index ? Colors.white : Colors.white70))),
          ),
        ),
      ),
    ]),
...
class PrimaryCTA extends StatelessWidget {
  ...
  const PrimaryCTA({super.key, required this.label, required this.onPressed, this.icon = Icons.flash_on});
  @override Widget build(BuildContext context) => FilledButton.icon(
    onPressed: onPressed, icon: Icon(icon), label: Text(label),
    style: FilledButton.styleFrom(
      backgroundColor: AppTheme.accent(), foregroundColor: Colors.black,
      padding: const EdgeInsets.symmetric(vertical: 16), textStyle: const TextStyle(fontSize: 16,fontWeight: FontWeight.w700),
