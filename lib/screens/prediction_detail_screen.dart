...
class PredictionDetailScreen extends StatefulWidget {
  ...
  const PredictionDetailScreen(
      {super.key, required this.symbol, required this.prediction});
...
class _PredictionDetailScreenState extends State<PredictionDetailScreen> {
        ...
        children: [
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    LinearProgressIndicator(
                        ...
                        minHeight: 8),
                    const SizedBox(height: 8),
                    Text('${confidencePct.toStringAsFixed(1)}% Confidence',
                        style: const TextStyle(
                            fontSize: 16, fontWeight: FontWeight.w600)),
                    const SizedBox(height: 12),
                    Text(_getConfidenceExplanation(confidencePct),
                        style:
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                  ...
                  children: [
                    const Row(children: [
                      Icon(Icons.auto_graph, color: Colors.orange),
                      SizedBox(width: 8),
                      Text('Model Analysis',
                          style: TextStyle(
                              fontSize: 18, fontWeight: FontWeight.bold))
                    ...
                    _buildModelRow(
                        '📈 Direction Model',
                        'Predicts $action with $directionProb% probability',
                        'Analyzed 64 candles of price action patterns'),
                    const Divider(height: 24),
                    _buildModelRow(
                        '💰 Return Model',
                        'Expected return: ${expReturnPct >= 0 ? '+' : ''}${expReturnPct.toStringAsFixed(2)}%',
                        'Based on historical momentum and trend strength'),
                    const Divider(height: 24),
                    _buildModelRow(
                        '📊 Volatility Model',
                        'Market volatility: ${volPct.toStringAsFixed(1)}%',
                        'Risk level: ${_getRiskLevel(volPct)}'),
                  ]),
            ),
          ),
          const SizedBox(height: 16),
            ...
            child: Padding(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    _buildIndicatorRow(
                        'Momentum', _getMomentumSignal(confidencePct)),
                    _buildIndicatorRow(
                        'Volatility', _getVolatilitySignal(volPct)),
                  ]),
            ),
          ),
          const SizedBox(height: 16),
          Container(
            ...
            child: const Row(children: [
              ...
              Expanded(
                  child: Text(
                      'This is an AI prediction, not financial advice. Always do your own research.',
  ...
  String _getConfidenceExplanation(double conf) {
    if (conf >= 50)
      return 'Strong signal - High probability of accurate prediction';
    if (conf >= 35)
      return 'Moderate signal - Reasonable confidence in prediction';
...
_NormalizedPred _normalize(dynamic pred, String symbol) {
  if (pred is ai.Prediction) {
    ...
    final int dirProb = () {
      ...
      if (action == 'SELL')
        return ((1.0 - pred.pBuy - (pred.pHold ?? 0.0)) * 100).round();
      // HOLD
  ...
  try {
    ...
    final double exp = (pred.expReturn ?? 0.0).toDouble();
    // Dacă pare fracțional mic, tratează ca % deja
    final double vol = (pred.volatility ?? 0.0).toDouble();
    final List probs = (pred.probs ?? const [33, 33, 33]);
    int dirProb = 0;
    if (action == 'BUY') {
      dirProb = (probs[0] as num).round();
    } else if (action == 'SELL')
      dirProb = (probs[2] as num).round();
    else
      dirProb = (probs[1] as num).round();
    return _NormalizedPred(
      action: action,
    ...
    return _NormalizedPred(
        ...
        expReturnPercent: 0,
        annVolPercent: 0,
        directionProbabilityPercent: 33);
  }
}
