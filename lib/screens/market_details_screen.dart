...
class _MarketDetailsScreenState extends State<MarketDetailsScreen> {
  ...
  @visibleForTesting final chartLoadingKey = const Key('market.chart.loading');
  ...
  void didChangeDependencies() {
    ...
    assert(() {
      ...
      if (!hasProvider) {
        throw FlutterError('MarketData provider missing. Folosește wrapWithMarketData(...) în teste.');
      }
      return true;
    }());
  }
  ...
  Future<void> _loadData() async {
    if (widget.loadDataFn != null) {
      ...
      setState(() {
        if (widget.forTest) {
          _klinesFuture = Future.delayed(const Duration(milliseconds: 100), () => List.generate(10, (i) => [0,0,0,0, 10000 + i.toDouble()]));
          ...
          Card(
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  const Text('Last price', style: TextStyle(fontSize: 14)),
                  Text(
                ...
                child: FutureBuilder<List<List<num>>>(
                  ...
                  builder: (context, snap) {
                    if (snap.connectionState == ConnectionState.waiting) {
                      return const Center(key: ValueKey('market.chart.loading'), child: CircularProgressIndicator());
                    }
                    if (snap.hasError) {
                      return Center(
                        child: Text(
              ...
              return Row(children: [
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: supported
                        ? () {
                            Navigator.push(context, MaterialPageRoute(builder: (_) => TradingModal(assetSymbol: symbol, isBuying: true)));
                          }
                        : null,
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    onPressed: supported
                        ? () {
                            Navigator.push(context, MaterialPageRoute(builder: (_) => TradingModal(assetSymbol: symbol, isBuying: false)));
                          }
                        : null,
