...
class RowRec {
  ...
  final int y;
  final double p;
  RowRec(this.ts, this.sym, this.y, this.p);
}

void usage() {
  print(
      'Usage: dart run tool/backtest_calibrate.dart --in <file.json|csv> [--bins 15] --calibrator <identity|platt|isotonic> --out <calibration.json>');
}

Future<void> main(List<String> args) async {
  final arg = Map.fromEntries(List.generate(
      args.length ~/ 2, (i) => MapEntry(args[i * 2], args[i * 2 + 1])));
  final input = arg['--in'];
  ...
  final meta = {
    ...
    'train_meta': {
      ...
      'window':
          '${DateTime.fromMillisecondsSinceEpoch(rows.first.ts).toUtc().toIso8601String()}..${DateTime.fromMillisecondsSinceEpoch(rows.last.ts).toUtc().toIso8601String()}',
  ...
  // artifacts report
  const reportPath = 'artifacts/calibration_report.json';
  ...
  await File(reportPath)
      .writeAsString(const JsonEncoder.withIndent('  ').convert(rep));
...
Future<List<RowRec>> _readRows(String path) async {
  ...
  if (path.endsWith('.json')) {
    ...
    return a.map<RowRec>((e) {
      final m = e as Map;
      return RowRec(m['timestamp'] as int, (m['symbol'] ?? '').toString(),
          (m['y_true'] as num).toInt(), (m['p_raw'] as num).toDouble());
    }).toList();
  } else {
    ...
    for (final l in lines.skip(1)) {
      final parts = l.split(',');
      out.add(RowRec(int.parse(parts[0]), parts[1], int.parse(parts[2]),
...
Calibrator _fitIsotonic(List<RowRec> train,
    {int bins = 15, int minPerBin = 0}) {
  ...
  final ys = <double>[];
  for (final b in rb) {
    xs.add(((b.lo + b.hi) / 2.0).clamp(0.0, 1.0));
    ys.add(b.meanTrue.clamp(0.0, 1.0));
  }
