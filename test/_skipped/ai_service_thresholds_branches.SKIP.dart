...
class _BuyModels implements ModelsAdapter {
  ...
  Future<({double? probUp, double? nextReturn, double? volatility})> predictAll(
      ...
      (probUp: 0.65, nextReturn: 0.02, volatility: 0.16); // HIGH - crypto volatility
...
class _HoldModels implements ModelsAdapter {
  ...
  Future<({double? probUp, double? nextReturn, double? volatility})> predictAll(
          Map<String, double> features) async =>
      (probUp: 0.50, nextReturn: 0.0, volatility: 0.06); // MEDIUM - crypto volatility
...
void main() {
  test('AIService → BUY branch at/above 0.60 (crypto threshold)', () async {
    ...
    expect(r.action, 'BUY');
    expect(r.confidence, greaterThanOrEqualTo(60)); // 0.65 → 65% (crypto-optimized)
    expect(r.volatility, 'HIGH'); // crypto volatility threshold >= 0.15
  });

  test('AIService → HOLD branch between thresholds (0.40 < prob < 0.60)', () async {
    final svc = AIService(
        client: _FakeClient(),
        fb: const FeatureBuilder(),
        models: _HoldModels());
