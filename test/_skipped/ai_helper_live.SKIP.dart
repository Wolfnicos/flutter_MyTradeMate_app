  ...
  group('AI Helper LIVE Data Tests', () {
    ...
    test('AI Service returnează predicții REALE pentru BTC', () async {
      ...
      try {
        ...
        print('✅ BTC Prediction LIVE: ${prediction.action} @ ${prediction.confidence.toStringAsFixed(1)}%');
      } catch (e) {
    ...
    test('Toate cele 5 crypto au predicții AI disponibile', () async {
      ...
      for (final crypto in supportedCrypto) {
        try {
          ...
          print('✅ ${crypto['name']}: ${prediction.action} (${prediction.confidence.toStringAsFixed(1)}%)');
          successCount++;
        } catch (e) {
          print('⚠️ ${crypto['name']}: Nu este disponibil pe acest environment');
