...
class _FakeClient implements BinanceClientLike {
  ...
  Future<List<List<num>>> klines(String s, String i, {int limit = 128}) async {
    return List.generate(limit, (k) => [0, 0, 0, 0, 1000 + k.toDouble(), 0, 0, 0, 0, 0, 0, 0]);
  }
  ...
  @override
  Future<Map<String, dynamic>> ticker24h(String symbol) async => {'lastPrice': 1111.0, 'prevClosePrice': 1111.0};
}
...
void main() {
  test('AIService → BUY branch', () async {
    final svc = AIService(client: _FakeClient(), fb: const FeatureBuilder(), models: _ModelsAdapterBuy());
  ...
  test('AIService → HOLD branch', () async {
    final svc = AIService(client: _FakeClient(), fb: const FeatureBuilder(), models: _ModelsAdapterHold());
    final r = await svc.inferForSymbol('BTCUSDT');
  ...
  test('AIService → SELL branch', () async {
    final svc = AIService(client: _FakeClient(), fb: const FeatureBuilder(), models: _ModelsAdapterSell());
    final r = await svc.inferForSymbol('BTCUSDT');
