...
class _SellModels implements ModelsAdapter {
  ...
  Future<({double? probUp, double? nextReturn, double? volatility})> predictAll(
      Map<String, double> features) async => (probUp: 0.40, nextReturn: -0.01, volatility: 0.02);
...
void main() {
  test('AIService selects SELL when probUp <= 0.45', () async {
    final svc = AIService(client: _OkClient(), fb: const FeatureBuilder(), models: _SellModels());
