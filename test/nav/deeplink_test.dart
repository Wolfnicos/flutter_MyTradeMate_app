...
Route<dynamic> _routes(RouteSettings s) {
  if (s.name?.startsWith('/market/') == true) {
    ...
    return MaterialPageRoute(builder: (_) => MarketDetailsScreen(symbol: sym, forTest: true));
  }
...
void main() {
  testWidgets('Deep link /market/BTCUSDT opens MarketDetails', (tester) async {
    await tester.pumpWidget(wrapWithMarketData(MaterialApp(onGenerateRoute: _routes, initialRoute: '/market/BTCUSDT')));
